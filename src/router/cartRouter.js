...
router.post('/:cid/products/:pid', async (req, res) =>{
    ...
    const response = await cartsManager.addProductToCartInMongo(Number(cid), Number(pid), Number(quantity))
    if(response.ok){
        res.status(200).send(response)
    }
    else{
...
router.put('/:cid/products/:pid/:units', async (req, res) =>{
    ...
    const response = await cartsManager.addProductToCartInMongo(Number(cid), Number(pid), Number(units))
    if(result.ok){
        res.status(200).send(response)
    }else{
        res.status(400).send(response)
